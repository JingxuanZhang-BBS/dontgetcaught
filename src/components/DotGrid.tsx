...
const DotGrid = ({
  ...
  // Pointer interactions — uses gsap tweens instead of InertiaPlugin
  useEffect(() => {
    const pushDot = (dot: Dot, pushX: number, pushY: number) => {
      ...
      // Scale push by resistance (higher = less movement)
      const factor = Math.min(1, 750 / Math.max(resistance, 1));
      gsap.to(dot, {
        xOffset: pushX * factor,
        yOffset: pushY * factor,
        duration: 0.15,
        ease: 'power2.out',
        onComplete: () => {
          gsap.to(dot, {
